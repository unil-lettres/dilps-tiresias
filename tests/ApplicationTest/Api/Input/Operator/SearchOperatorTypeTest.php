...
class SearchOperatorTypeTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function providerSearch(): array
    {
        return [
            ...
            'search predefined joins' => [Card::class, 'foo', '(a.id LIKE :filter1 OR a.dating LIKE :filter1 OR a.code LIKE :filter1 OR a.name LIKE :filter1 OR a.street LIKE :filter1 OR a.locality LIKE :filter1 OR a.addition LIKE :filter1 OR a.expandedName LIKE :filter1 OR a.material LIKE :filter1 OR a.technique LIKE :filter1 OR a.objectReference LIKE :filter1 OR a.corpus LIKE :filter1 OR institution1.name LIKE :filter1 OR artist1.name LIKE :filter1 OR country1.name LIKE :filter1)'],
        ];
    }
}

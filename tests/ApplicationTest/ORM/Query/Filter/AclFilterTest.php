    ...
    public function providerFilter(): array
    {
        return [
            ...
            'student can access collections that are his own or are member' => [
                'student',
                Collection::class,
                'test.id IN (SELECT collection.id FROM collection WHERE (collection.visibility IN (\'member\')) OR (collection.owner_id = \'1003\' OR collection.creator_id = \'1003\'))',
            ],
            'administrator can access collections that are his own or are administrator or member' => [
                'administrator',
                Collection::class,
                'test.id IN (SELECT collection.id FROM collection WHERE (collection.visibility IN (\'member\', \'administrator\')) OR (collection.owner_id = \'1000\' OR collection.creator_id = \'1000\'))',
            ],
